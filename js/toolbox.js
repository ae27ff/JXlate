...
function tbox_addtooln(i) {
    tbox_addtool(tools[i][0], tools[i][1]);
}
...
function tbox_action_case() {
    if (tbox_case)
        tbox_textarea.value = tbox_textarea.value.toUpperCase();
    else
        tbox_textarea.value = tbox_textarea.value.toLowerCase();
    tbox_case = !tbox_case;
}
