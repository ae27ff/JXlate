if (typeof addcredits === 'function')
    addcredits("xlate.js", 10, "crashdemons", "Binary Translation library for numeral systems (extensions added for encoding libraries)");
...
function fill_bases() {
    for (var b = 36; b >= 0; b--)
        base_charsets[b] = b32hex.substr(0, b).split("");
...
function array_base2base(a, baseFrom, baseTo) {//convert arrays of numerals from one base to another - implements support for Base64
    ...
    for (var i = 0, len = a.length; i < len; i++)
        a[i] = base2base(a[i], baseFrom, baseTo);//do a base conversion on each array element (unit in the From Base) to the To base.
    return a;
}
